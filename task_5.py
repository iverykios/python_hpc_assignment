...
Combined script for Task 5:
- Mode 1: Parallel floorplan simulation with static scheduling
- Mode 2: Analysis of timing results and speedup calculations

Usage:
  For simulation:
    python task5.py simulate <N> <num_workers>
    
  For analysis:
    python task5.py analyze <results_dir>
"""
...
def run_simulation(N, num_workers):
    ...
    print(f"Processing {N} floorplans using {num_workers} workers...")
    
    # Record start time
    start_time = time.time()
    
    # Process floorplans in parallel using a Pool of workers
    with mp.Pool(processes=num_workers) as pool:
        results = pool.map(process_floorplan, building_ids)
...
def main():
    if len(sys.argv) < 2:
        print("Usage:")
        print("  For simulation: python task5.py simulate <N> <num_workers>")
        print("  For analysis:   python task5.py analyze <results_dir>")
        sys.exit(1)
    ...
    if mode == "simulate":
        if len(sys.argv) < 4:
            print("Usage for simulation: python task5.py simulate <N> <num_workers>")
            sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print("Usage for analysis: python task5.py analyze <results_dir>")
            sys.exit(1)
